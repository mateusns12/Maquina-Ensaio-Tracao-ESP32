...
void parametros (void *pvParameters){
  ...
  bool enable = 0;
  while(true){
    f = 9.81*scale.get_value(1)/-22800;
    portENTER_CRITICAL(&mux);
      d = distance;
      enable = pt;
    portEXIT_CRITICAL(&mux);
    ...
    if(enable==1){
      //Serial.println(String(d)+"\t"+String(f));
      //Serial.println(String(deformP)+"\t"+String(carga));
      //Serial.println(String(d)+"\t"+String(f)+"\t"+String(deformP)+"\t"+String(carga));
      Serial.print(d);
      Serial.print("\t");
      Serial.print(f);
      Serial.print("\t");
      Serial.print(deformP,4);
      Serial.print("\t");
    ...
    while(true) { 
      ...
      if (packet < 0) {
        ...
        portENTER_CRITICAL(&mux);
          distance = y; 
        portEXIT_CRITICAL(&mux);           
        //Serial.println(String(y)+"\t"+String(distance));
      }esp_task_wdt_reset();
    }esp_task_wdt_delete(NULL);
    vTaskDelete( NULL ); 
}
...
void CheckSerialSwitchCase() 
{
  if (Serial.available()) { 
    ...
    case 'l':    
      Serial.print("LRT: ");
      Serial.print(lrt,4);
      Serial.print("\tA. Inicial: ");
      Serial.print(a_inicial,4);
      Serial.print("\tC. Inicial: ");
      Serial.print(c_inicial,4);
      Serial.println("");
      break;
    ...
    case 'r':    
      if(flag==0){
        ...
        //vTaskDelay(200/portTICK_PERIOD_MS);
        //gpio_set_level(ENABLE,1);
        digitalWrite(GPIO_PWM0B_OUT, HIGH);
      }else{
    ...
    case 't': //inicia teste
      Serial.println("");
      digitalWrite(GPIO_PWM0A_OUT, HIGH);
      lrt = 0;
      xTaskCreatePinnedToCore(mainTask,"mainTask",10000,NULL,3,&task_main,taskCoreZero);
      vTaskDelay(200/portTICK_PERIOD_MS);
      //xTaskCreatePinnedToCore(teste,"teste",10000,NULL,3,&task_teste,taskCoreZero);
      //vTaskDelay(200/portTICK_PERIOD_MS);
      xTaskCreatePinnedToCore(parametros,"parametros",10000,NULL,3,&task_parametros,taskCoreZero);
      vTaskDelay(200/portTICK_PERIOD_MS);
    ...
    case 's': //para teste      
      ...
      esp_task_wdt_delete(task_main);
      vTaskDelete(task_main);
      Serial.println("main deletada");
      esp_task_wdt_delete(task_parametros);
      vTaskDelete(task_parametros);      
      Serial.println("parametro deletado");
